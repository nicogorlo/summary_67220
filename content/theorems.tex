...
\begin{definition}[L4.2]{Strict and strong convexity.}
    ...
    \begin{itemize}[leftmargin=*]
        ...
        \item For $f$ differentiable at $x_0 \in \Omega$ and $\Omega$ open, strong convexity is equivalent to $f(y) \geq f(x_0) + \langle \nabla f(x_0), y-x_0\rangle + \frac{\mu}{2}\|y-x_0\|_2^2 \text{ } \forall y\in\Omega$.
        \item PSet4.1a. For $f$ differentiable and $\mu$-strongly convex, $\Omega$ open: $f(y)\leq f(x) + \langle \nabla f(x), y-x\rangle + \frac{1}{2\mu}\|\nabla f(x)-\nabla f(y)\|_2^2 \quad \forall x,y\in \Omega$.
    \end{itemize}
\end{definition}
...
\begin{theorem}[L7.1]{Normal cone to the intersection of linear inequalities. (rewriting L3.1,L5.3)}
    ...
    $$
    \vspace{-4pt}\\
    Then, the normal cone at any $x \in \Omega$ is given by\\ {\footnotesize$\mathcal{N}_{\Omega}(x)=\left\{\sum_{i=1}^r \mu_i a_i+\sum_{j \in I(x)} \lambda_j c_j: \mu_i \in \mathbb{R}, \lambda_j \in \mathbb{R}_{\geq 0}\right\}$
    \vspace{-4pt}\\
    $$
    \begin{aligned}
...
\begin{remark}[]{}
    {\footnotesize%
    \textbf{Additional Note:} (PSet2.3a) Let $f:\Omega \rightarrow \mathbb{R}$ be convex, $\Omega$ convex. Consider any point $x$ in interior of $\Omega$. Then $f$ is continuous at $x$. Th4.3.1 restated:
    \vspace{-4pt}\\ 
    $$
    \begin{aligned}
        & f(x\pm t \epsilon)-f(x) \leq t(f(x \pm \epsilon)-f(x)) \\
        & f(x\pm t \epsilon)-f(x) \geq t(f(x)-f(x\mp \epsilon))
    \end{aligned}
    $$
    \vspace{-3pt}
    }
...
\begin{remark}[X]{Additional notes:}\\
    ...
    - PSet1.5a: Let the function $f: \mathbb{R}^n \rightarrow \mathbb{R}$ be differentiable and bounded below. For any $\epsilon>0$, there exists some point $x_\epsilon \in \mathbb{R}^n$ such that $\left\|\nabla f\left(x_\epsilon\right)\right\|_2 \leq \epsilon$.
    }
\end{remark}