...
\begin{remark}[L5.4]{Ellipsoid method.}
    at each iteration, start at a point $c_t$ and a search space (that initially is $\Omega_1 = \Omega$).
    \begin{itemize}[leftmargin=*]
        \item if $c_t$ is not in the search space, then use a separating hyperplane to cut the search space in half
        \item if $c_t$ is in the search space, intersect the search space with $H_t = \{x \in \mathbb{R}^n : \langle \nabla f(c_t), x - c_t\rangle \leq 0\}$ (to make the search space contain only points x such that $f(x) \geq f(c_t)$ by Th.L4.3)
        \item set $\Omega_{t+1} = H_t \cap \Omega_t$
    \end{itemize}
\end{remark}
...
\begin{theorem}[L9.1]{Normal cone to the intersection of a hyperplane and a closed convex set.}
